...
if (isset($_SESSION['pseudo']) OR isset($_SESSION['email'])){
	...
	function generate_img_list($bdd, $lan_id){
		...
		$dossier_thumb = "../img/uploads/previous_lan/thumbnails/";
		$dossier_gal = "../img/uploads/previous_lan/";

		$reqx = $bdd->prepare("SELECT thumb_name FROM user_img WHERE img_type = 'gallery' AND lan_id = :lan_id");
		$reqx->execute(array(
		'lan_id' => $lan_id));
		...
		{
			$this_created_img_div = "<div id=\"prev_lan" . $lan_id . "_pic_" . $x . "\" href=\"#\" class=\"gamesImg add\" style=\"background-image: url('" . $dossier_thumb . $img_list[$x]['thumb_name'] . "')\"></div>";
		...
		$LANs_table = array (
		'10' => "LAN 1nd14n 5umm3r - 74r637 : L457 88Q @Gunther's, 25-26-27/10/2019",
		'9' => "LAN 5pr1n6 is C0m1n6 @Gunther's, 10-11-12/05/2019",
